...
int s21_mul(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  if (check_decimal(value_1) || check_decimal(value_2) || result == NULL) {
    ...
    int exponent_1 = get_decimal_exponent(value_1),
        exponent_2 = get_decimal_exponent(value_2);
...
int s21_mul_handle(s21_big_decimal big_result, int result_exponent,
  ...
  if (!s21_is_full_equal_zero(big_result.decimal[1]) ||
    ...
    *result = big_result.decimal[0];
    ...
    if (s21_is_equal(*result, get_decimal_with_int_value(0)) &&
        s21_is_equal(div_remainder.decimal[0], get_decimal_with_int_value(0))) {
...
s21_big_decimal s21_big_mul(s21_big_decimal value_1, s21_big_decimal value_2) {
  ...
  max_index = max_index == -1 ? get_count_full_digits(value_2.decimal[0]) - 1
