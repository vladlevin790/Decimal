...
int ffflag = 0;
    ...
    if (sign_vol1 ==1 && sign_vol2 == 1) { // если оба числа отрицательные
        if (super_comparison_of_numbers(val_1, val_2) == 0) { // определяем какой из чисел больше 
            if (exp1>=exp2) exp = exp1; else exp = exp2; // востановили экспоненту 23.07.2024
            res = s21_big_sub(val_1, val_2);
            // super_logic_sub(val_1, val_2, &res);
            transpot_to_Oleg_big_decimal(&res);
            ...
            set_sign_sub(&res.decimal[0], sign_vol1);
            set_decimal_exponent(&res.decimal[0], get_decimal_exponent(val_1.decimal[0]));
            if (s21_big_decimal_cheak_on_zero(res, 1) == 1) ffflag=1; // проверяем умещается ли ответ в один децимал
            else  transpot_to_Oleg_big_decimal(&res);
    } else {
            super_logic_sub(val_2, val_1, &res);
            sign_vol1 = 1; // так как оба числа положительные и первое число val1 меньше vol2 знак ответа отрицательный
            set_sign_sub(&res.decimal[0], sign_vol1);
            set_decimal_exponent(&res.decimal[0], get_decimal_exponent(val_1.decimal[0]));
            if (s21_big_decimal_cheak_on_zero(res, 1) == 1) ffflag=1;
            else  transpot_to_Oleg_big_decimal(&res);
            }
            ...
            sign_vol1 = 0;
            res = s21_big_decimal_add_mirror(val_2,val_1);
            set_sign_sub(&res.decimal[0], 1); ///!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
            sign_vol1 = 1;
            set_decimal_exponent(&res.decimal[0], get_decimal_exponent(val_1.decimal[0]));
            if (s21_big_decimal_cheak_on_zero(res, 1) == 1) ffflag=1;
            else  transpot_to_Oleg_big_decimal(&res);
    ...
    if (s21_big_decimal_cheak_on_zero(res, 0)== 0){ //проверка ответа на ноль если ноль и имеется знак минус меняем на плюс
        ...
        if (ffflag ==1) *result = res.decimal[0];
        else *result = scale_down(res, sign_vol1, exp);
        // printf("-EXPONENT_N-\n");
...
int s21_big_decimal_cheak_on_zero(s21_big_decimal value, int mode){ // mode = 0 полная проверка mode = 1 проверка что заполнен только decimal[0]
    if (mode == 0) {
        ...
        for (int i = 0; i<3; i++){
            if (value.decimal[1].bits[i] == 0) continue;
            else return 0;
        }
        for (int i = 2; i>=0; i--){
            if (value.decimal[0].bits[i] != 0)  return 1;
        }
        return 0;
    }
    return -1; //ошибка
