...
s21_big_decimal init_s21_big_decimal(s21_decimal value);
...
// БЛОК РАБОТЫ С ЭКСПОНЕНТОЙ УРАВНИВАНИЕ ЭКСПОНЕНТЫ
    ...
    printf("\n EXP1 = %d  EXP2  = %d\n", exp1, exp2);
    if (exp1 > exp2) {
        val_1 = init_s21_big_decimal(*value_1);
        val_2 = init_s21_big_decimal(*value_2);

        int i = exp1 - exp2 - 1; // нужна для правильной работы s21_big_decimal_add_mirror
        s21_big_decimal temp = val_2; // нужна для правильной работы s21_big_decimal_add_mirror
        while (i >= 0) {
        ...
        val_1 = init_s21_big_decimal(*value_1);
        val_2 = init_s21_big_decimal(*value_2);
        int i = exp2 - exp1 - 1; // нужна для правильной работы s21_big_decimal_add_mirror
        s21_big_decimal temp = val_1; // нужна для правильной работы s21_big_decimal_add_mirror
        while (i >= 0) {
